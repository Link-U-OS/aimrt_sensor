...
CONFIG_FILE_PATH="../config/hal_d415/hal_d415_config.yaml"
...
LOGFILE="/agibot/log/hal_d415/hal_d415.log"
...
Usage() {
    cat <<EOF
    start_hal_d415.sh [options]
    -p: debug port
    -g: run with debug mode
    -c: using custom config file(absolute path)
    -h: help
    e.g.: 1) start_hal_d415.sh -c /home/agibot/hal_d415_config.yaml
EOF
    exit 1
}
...
# Execute hal_d415 binary with background execution to capture PID
echo -e "\n$(get_time) INFO: ✊ Start running hal_d415 ..."
chmod a+x ./hal_d415
export LOG_PATH=${LOG_PATH:-"../log"}
export AGIBOT_ENABLE_LOG_CONTROL='false'
# Start program and get PID
RMW_LIBRARY_PATH=$(pwd)/librmw_fastrtps.so LD_LIBRARY_PATH=./:$LD_LIBRARY_PATH ${DEBUG_COMMAND} ./hal_d415 --cfg_file_path=$CONFIG_FILE_PATH &
PID=$!
...
if [ $EXIT_CODE -eq 0 ]; then
    echo -e "\n$(get_time) INFO: ✅ hal_d415 executed successfully"
else
    echo -e "\n$(get_time) ERROR: ❌ hal_d415 execution failed, exit code: $EXIT_CODE"
    exit 1
fi
popd || exit
